...
>数组索引不能超出数组长度。如果是明显的常量越界，编译器可能在编译期直接报错。如果索引值在运行时才确定，运行时越界会触发`panic`。`panic`即程序崩溃的意思，会在[[13、错误处理|错误处理]]详细介绍。
...
文档注释中的 Rust 代码块还可以作为**文档测试**，也就是`doctest`，详见[[15、测试（待补充）|测试]]。
