...
后续会介绍的[[Rust/notes/Rust基础/6、结构体|结构体]]、[[Rust/notes/Rust基础/7、枚举|枚举]]、[[Rust/notes/Rust基础/12、特征基础|特征基础]] 等类型名，则通常采用**帕斯卡命名法**（`Pascal Case`），即每个单词首字母大写，单词之间不使用下划线。
...
| `{:?}`  | `Debug`   | 开发调试输出     |
| `{:#?}` | `Debug`   | 美化后的多行调试输出 |
这里的`Display`、`Debug`是`Trait`（参考[[Rust/notes/Rust基础/12、特征基础|特征基础]]）。这里只需要知道，打印数组使用`{:?}`，因为数组没有实现`Display`。

在后面介绍[[Rust/notes/Rust基础/6、结构体|结构体]]时，可以直观比较`{:?}`和`{:#?}`的区别。`{:#?}`会多行输出结构体的每一个字段且加上缩进，而`{:?}`则是一行显示结构体。
...
>数组索引不能超出数组长度。如果是明显的常量越界，编译器可能在编译期直接报错。如果索引值在运行时才确定，运行时越界会触发`panic`。`panic`即程序崩溃的意思，会在[[Rust/notes/Rust基础/15、错误处理|错误处理]]详细介绍。
...
文档注释中的 Rust 代码块还可以作为**文档测试**，也就是`doctest`，详见[[Rust/notes/Rust基础/16、测试|测试]]。
