...
- `stats` 用于统计日志级别数量，例如 `INFO`、`WARN`、`ERROR`。
...
[2026-07-20T09:31:12Z WARN rlog::network] connection is slow
...
| 两条 `WARN` | 验证警告级别过滤 |
| 两条 `ERROR` | 验证错误级别过滤 |
| 多条包含 `connection` 的日志 | 验证 `--contains connection` |
| 不同模块名 | 为后续解析模块预留测试数据 |
...
| 日志级别解析 | 从每一行日志中识别 `INFO`、`WARN`、`ERROR` |
...
| `stats.rs` | 统计 `INFO`、`WARN`、`ERROR` 数量 |
