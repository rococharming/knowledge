...
不同网站提供的数据并不完全相同，使用 Web Clipper 页面的 `...` 图标可以访问当前页面的变量：
...
HTML网页的`<head>`中通常包含`<meta>`元素，用于描述页面标题、摘要、作者、摘要图等信息。
...
提示变量只有在启用并配置解释器后才能使用。具体配置方法参见[[#七、解释器]]。
...
解释器提示变量则使用自然语言指令，让模型对网页内容进行总结、翻译、提取或改写。具体语法和用法参见[[#2.8、提示变量]]。
