...
如果某个领域目录已经存在但结构不完整（比如缺少 `wiki/` 子目录、`index.md` 或 `log.md`），直接对该领域调用 init-domain 即可，它会检查缺失项并补充，**不会覆盖你已有的内容**。

> "帮我完善 AI 领域" 或 "检查一下心理学目录还缺什么"
...
|---|---|---|
| **创建/完善领域** | "创建心理学领域" 或 "帮我完善 AI 领域" | 完整的目录结构、个性化的 `CLAUDE.md`，已有内容不会被覆盖 |
...
| **健康检查** | "lint 一下 wiki" | 死链、孤立页面、矛盾、陈旧内容、缺失交叉引用、重要概念缺页、数据空白的报告 |
...
- `wiki/` — LLM 维护，你主要阅读和审阅；若手动修改，应让 LLM 随后检查链接、index、log 与内容一致性
- `notes/` — 你专属，LLM 绝不触碰
