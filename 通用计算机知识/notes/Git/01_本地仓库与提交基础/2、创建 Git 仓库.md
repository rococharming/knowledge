...
暂存和提交流程见 [[通用计算机知识/notes/Git/01_本地仓库与提交基础/4、查看状态、暂存和提交|查看状态、暂存和提交]]。
...
因此即使当前位于 `src/`，执行 `git status` 仍属于这个项目仓库。
...
本课只确认普通文件和 `.git/` 数据库的分工，不修改文件，也不创建提交。目标状态是：仓库仍然保持 clean，下一课开始制造文件状态变化。
