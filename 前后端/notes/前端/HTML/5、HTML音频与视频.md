...
如果不写 `preload`，标准没有规定所有浏览器必须使用同一个默认值，而是交给浏览器自己决定；规范建议的折中默认倾向是 `metadata`。因此实际开发中不要依赖省略时的默认行为，想表达预加载策略时最好显式写出 `preload="metadata"`、`preload="none"` 或 `preload="auto"`。
...
MIME 的全称是 Multipurpose Internet Mail Extensions，直译是“多用途互联网邮件扩展”。它最早用于邮件附件类型标识，后来也被 Web 广泛使用，用来说明一个资源的内容类型，例如 `audio/mpeg` 表示 MP3 音频，`video/mp4` 表示 MP4 视频。
