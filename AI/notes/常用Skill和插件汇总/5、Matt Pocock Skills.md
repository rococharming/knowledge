...
如果希望通过命令行安装这套 Skills，可以使用 `npx`：

```shell
npx skills@latest add mattpocock/skills
```

![[assets/Pasted image 20260727003025.png|600]]

安装时有`Mattpocock Skills`和`Other`两套技能选择，这里只选择`Mattpocock Skills`。

在`Mattpocock Skills`这套技能中，也按需选择自己想要的技能。其中`setup-matt-pocock-skills`技能是必须的，它用于初始化项目。
...
`npx` 安装的特点是：既可以面向具体项目使用，也可以按工具支持的方式安装到更通用的位置。它更适合习惯通过命令行管理 Skill、并希望明确控制安装内容的场景。

## 2、Claude Code 托管安装

如果只想使用随版本更新的托管包，可以在 Claude Code 中通过 `/plugin` 安装：
...
Claude Code 插件安装的特点是：整体 Skill 集保持只读并跟随发布版本更新，适合不想维护本地副本、但希望在 Claude Code 内直接调用这些 Skill 的场景。

## 3、ChatGPT 桌面端安装

在 ChatGPT 桌面端里，可以通过添加插件市场来安装 `mattpocock/skills`。

![[assets/Pasted image 20260727003511.png|600]]

![[assets/Pasted image 20260727003547.png|600]]

添加插件市场后，就能在插件中检索到 Matt Pocock Skills 并安装了：

![[assets/Pasted image 20260727003711.png|600]]
...
![[assets/Pasted image 20260727005224.png|600]]
...
![[assets/Pasted image 20260727005552.png|600]]
...
![[assets/Pasted image 20260717145925.png|500]]
...
![[assets/Pasted image 20260727005850.png|600]]
...
![[assets/Pasted image 20260727010242.png|600]]
...
![[assets/Matt Pocock Skills workflow handdrawn.png|800]]
...
![[assets/Matt Pocock Skills idea to ship handdrawn.png|800]]
