...
- **Slash命令**：`/`菜单中既有内置固定逻辑命令，也有`Skill`命令（详见[[2、Slash命令]]）
...
| 级别  |                 路径                 |   适用范围    | 覆盖优先级  |
| :-: | :--------------------------------: | :-------: | :----: |
| 企业级 |   参考[[设置]]的**managed settings**    |  组织内所有用户  |   最高   |
...
来自`--add-dir`目录的`CLAUDE.md`文件默认不加载，需设置`CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD=1`（[[3、记忆机制#^566a05|详见记忆机制]]）。
