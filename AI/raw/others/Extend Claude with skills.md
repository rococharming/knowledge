...
published:
created: 2026-05-28
description: "Create, manage, and share skills to extend Claude's capabilities in Claude Code. Includes custom commands and bundled skills."
tags:
  - "clippings"
---
# AI摘要

## 为什么值得读：本文是 Claude Code 技能的权威官方文档，系统性地解释了如何通过创建 `.md` 文件来扩展 Claude 的能力。它解决了“重复粘贴指令”的痛点，将常规操作（如代码审查、部署）标准化为可复用的技能，并提供了从入门到高级的完整指南。

## 是什么：Claude Code 技能是一种通过编写 `SKILL.md` 文件来定义的可复用指令集。它允许用户和 Claude 通过 `/技能名` 调用，或让 Claude 在相关场景下自动加载。技能支持参数传递、子代理执行、动态上下文注入等高级功能。

## 如何做：
1. **创建技能目录和文件**：在 `~/.claude/skills/`（全局）或 `.claude/skills/`（项目级）下创建技能目录，并放入 `SKILL.md` 文件。
2. **编写 `SKILL.md`**：使用 YAML 开头（建议包含 `description` 字段）和 Markdown 正文。正文应简洁，给出明确的步骤指令。
3. **配置行为**：通过 `disable-model-invocation`（仅用户调用）、`user-invocable`（仅 Claude 调用）、`allowed-tools`（预授权工具）等字段控制技能行为。
4. **使用高级特性**：可添加 `context: fork` 在子代理中运行技能，使用 `!\\`command\\`\\" 语法注入动态上下文，通过 `$ARGUMENTS` 传递参数。
5. **共享与分发**：将 `.claude/skills/` 提交到版本控制（项目级），或通过插件/企业托管设置进行分发。
...
| `model` | No | Model to use when this skill is active. The override applies for the rest of the current turn and is not saved to settings; the session model resumes on your next prompt. Accepts the same values as [`/model`](https://code.claude.com/docs/en/model-config), or `inherit` to keep the active model. |
...
<html><head>
  ...
  <script>
    ...
    function fmt(b) 
    function esc(s) [c])); }}
    function render(node, parent) </span><span class="size">$</span></summary>\`;
        const ul = document.createElement('ul'); ul.className = 'tree';
        node.children.sort((a,b) => (b.children?1:0)-(a.children?1:0) || a.name.localeCompare(b.name));
        node.children.forEach(c => render(c, ul));
        det.appendChild(ul);
        const li = document.createElement('li'); li.appendChild(det); parent.appendChild(li);
      }} else "></span>$<span class="size">$</span>\`;
